...
def render_message(input_dir, page_elements, people, message, timezone=None):
  ...
  with tag('div', klass='message_container'):
      ...
      with tag('div', klass='message_box'):
          ...
          if message['text']:
              deleted = message['text'] == 'This message was deleted'
              with tag('span', klass='message'):
                  ...
                  for t, style in text_parts:
                      with tag('span'):
                          if deleted:
                            doc.attr('style="font-weight: %s; color: #FF0000;"' % (style))
                            text(t)
                          else:
...
def render(path_):
  ...
  if not os.path.exists(os.path.join(path_, 'people.json')) or \
     not os.path.exists(os.path.join(path_, 'messages.json')) or \
     not os.path.exists(os.path.join(path_, 'group_info.json')):
      print("Missing files!")
      sys.exit(1)

  with open(os.path.join(path_, 'people.json')) as fp:
      people = json.load(fp)

  with open(os.path.join(path_, 'messages.json')) as fp:
      messages = json.load(fp)

  with open(os.path.join(path_, 'group_info.json')) as fp:
          ...
          with tag('div', id='container'):
              ...
              for message in messages:
                  ...
                  if people[message['author']]['name'] == __SYSTEM__:
                      ...
                      render_message(path_, page_elements, people,
                                     message, tz)


  # Save rendered files  
  with open(os.path.join(path_, 'index.html'), 'w') as fp:
      fp.write(indent(doc_))

  with open(os.path.join(path_, 'main.css'), 'w') as fp:
      fp.write(css_file())

  root_path = os.path.realpath(__file__)
  assets_dir = os.path.join(os.path.dirname(root_path), 'assets')
  project_assets_dir = os.path.join(path_, 'assets')
